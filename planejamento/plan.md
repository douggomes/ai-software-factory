...
Cada arquivo segue o [template normativo](task-template.md) e é validado por `python3 scripts/validate_tasks.py`. Somente `TASK-001` está `ready`; uma task planejada recebe baseline SHA e é ativada apenas após todas as dependências terem evidência aprovada.
...
5. O agente executa os gates globais, revisa o diff e entrega relatório/evidências; quando autorizado, publica a task por PR com destino a `dev`, nunca `main`.
6. A Factory/humano valida Definition of Done, segurança e evidências, integra em `dev` e somente então pode ativar a próxima task.
7. A última task de cada versão executa o gate/dossier; promoção ocorre por PR `dev → main` e publicação/tag continuam humanas.
8. Decisão que altere invariantes, contratos públicos, autoridade ou trust boundary exige ADR/task próprios, nunca escolha local do agente.

O [template normativo](task-template.md) exige Definition of Ready, precondições, baseline, paths, interfaces, defaults, riscos, critérios identificados e a correspondência exata AC → comando → teste → evidência. O validador documental bloqueia task inconsistente antes da execução.
