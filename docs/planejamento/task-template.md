...
1. `comando seguro executado pelo desenvolvedor`
   Esperado: resultado visual/funcional que confirma a entrega sem depender apenas dos testes.
2. `segundo comando quando necessário`
   Esperado: comportamento negativo ou limite importante observado manualmente.

Ao concluir a task, o agente executa `python3 scripts/show_manual_validation.py TASK-NNN` para imprimir esta seção no terminal antes da publicação automática.
...
Liste os artifacts mínimos que permitem a outro agente reproduzir a decisão sem transcript ou conhecimento tácito. Após os gates, o agente cria commit, publica a branch e abre/atualiza automaticamente um draft PR para `dev` com `gh`, incluindo a saída da validação manual.
