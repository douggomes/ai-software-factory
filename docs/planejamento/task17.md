...
1. `uv run pytest tests/integration/test_repair_loop.py::test_gate_failure_repair_and_revalidate -q`
   Esperado: Repair gera nova attempt e novos gates antes de success.
2. `uv run pytest tests/chaos/test_repair_restart.py -q`
   Esperado: Restart respeita o budget persistido sem attempt extra.

O agente imprime esta seção com `python3 scripts/show_manual_validation.py TASK-017` antes de publicar o draft PR.
