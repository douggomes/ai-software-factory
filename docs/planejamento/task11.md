...
1. `uv run aif workers health`
   Esperado: Exibe Closed/Open/HalfOpen e expiração sem modificar health.
2. `uv run pytest tests/integration/test_provider_health_store.py::test_single_half_open_probe -q`
   Esperado: Somente uma probe HalfOpen é concedida.

O agente imprime esta seção com `python3 scripts/show_manual_validation.py TASK-011` antes de publicar o draft PR.
