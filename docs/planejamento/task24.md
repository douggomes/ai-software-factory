...
1. `uv run aif telemetry check`
   Esperado: Emite trace/métrica local sem prompt, diff ou segredo.
2. `uv run pytest tests/release/test_v0_4.py -q`
   Esperado: No-op parity, MCP e observabilidade passam no gate V0.4.

O agente imprime esta seção com `python3 scripts/show_manual_validation.py TASK-024` antes de publicar o draft PR.
