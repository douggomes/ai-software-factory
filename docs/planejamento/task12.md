...
1. `uv run pytest tests/integration/test_continuation.py::test_failover_preserves_partial_work -q`
   Esperado: O segundo fake continua no mesmo worktree e preserva o diff parcial.
2. `uv run pytest tests/chaos/test_continuation_boundary.py -q`
   Esperado: Reinício no boundary não duplica package ou attempt.

O agente imprime esta seção com `python3 scripts/show_manual_validation.py TASK-012` antes de publicar o draft PR.
