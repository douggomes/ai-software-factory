...
1. `uv run aif route explain examples/specs/ticket-feature.md --task TASK-001`
   Esperado: Mostra features, dataset hash, pesos, exclusões, scores e fallback.
2. `uv run pytest tests/security/test_router_poisoning.py -q`
   Esperado: Histórico envenenado não altera elegibilidade, permissões ou budgets.

O agente imprime esta seção com `python3 scripts/show_manual_validation.py TASK-029` antes de publicar o draft PR.
