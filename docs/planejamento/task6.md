...
1. `uv run pytest tests/integration/test_process_runner.py::test_arguments_are_literal_and_policy_enforced -q`
   Esperado: Metacaracteres permanecem literais e paths/executáveis proibidos são negados.
2. `uv run pytest tests/integration/test_process_runner.py::test_timeout_kills_process_group -q`
   Esperado: Timeout encerra pai e filhos sem processos órfãos.

O agente imprime esta seção com `python3 scripts/show_manual_validation.py TASK-006` antes de publicar o draft PR.
