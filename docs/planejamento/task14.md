...
1. `uv run pytest tests/release/test_v0_1.py -q`
   Esperado: O gate offline completo da V0.1 passa.
2. `uv run aif run examples/specs/v0.1-demo.md --task TASK-001 --worker fake-success`
   Esperado: O primeiro run documentado pode ser reproduzido sem tokens.

O agente imprime esta seção com `python3 scripts/show_manual_validation.py TASK-014` antes de publicar o draft PR.
