...
1. `uv run aif context build <RUN-ID>`
   Esperado: Gera manifest canônico com path, razão, tamanho, hash e omissões.
2. `uv run aif context diff <RUN-A> <RUN-B>`
   Esperado: Explica diferenças de contexto sem consultar LLM.

O agente imprime esta seção com `python3 scripts/show_manual_validation.py TASK-019` antes de publicar o draft PR.
