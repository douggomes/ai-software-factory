...
- [x] **AC-001** — `uv sync --locked` e `aif --version` funcionam em checkout limpo.
- [x] **AC-002** — `aif doctor --json` é determinístico, offline e representa provider ausente como `unavailable`.
- [x] **AC-003** — Config inválida falha antes de efeitos e todos os gates de bootstrap passam.
...
1. `uv run aif --version`
   Esperado: Exibe a versão instalada e retorna código 0.
2. `uv run aif doctor --json`
   Esperado: Retorna JSON estável com Python, uv, Git, SQLite e providers; não executa modelos sem --live.

O agente imprime esta seção com `python3 scripts/show_manual_validation.py TASK-001` antes de publicar o draft PR.
