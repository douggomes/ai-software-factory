...
1. `uv run pytest tests/unit/core/test_state_machine.py -q`
   Esperado: Todas as transições e invariantes do Core passam deterministicamente.
2. `uv run lint-imports`
   Esperado: Confirma que o Core não importa adapters, ORM ou SDKs.

O agente imprime esta seção com `python3 scripts/show_manual_validation.py TASK-003` antes de publicar o draft PR.
